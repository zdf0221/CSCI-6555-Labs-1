public class Function {
    ...
    public static EulerAngle eInterpolation(String splineType,float t,int pointIndex){
        float[][] MS;
        ...
        float[][] G = new float[4][6];
        for (int i = 0; i <G.length ; i++) {
            //4 points
            G[i] = GlobalVariables.points[pointIndex+i];
        }
        return new EulerAngle(blend(T,MS,G));
    }
    ...
    public static Quaternion qInterpolation(String splineType,float t,int pointIndex){
        ...
        for (int i = 0; i <G.length ; i++) {
            //4 points
            EulerAngle e = new EulerAngle(GlobalVariables.points[pointIndex+i]);
